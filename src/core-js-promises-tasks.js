...
function getFirstResolvedPromiseResult(promises) {
  return new Promise((resolve, reject) => {
    Promise.all(
      ...
      const resolvedResult = results.find(
...
function queuPromises(promises) {
  const processPromise = async (result, promise) => {
    const value = await promise;
    return result + value;
  };

  return promises.reduce(
    async (previousPromise, nextPromise) =>
      processPromise(await previousPromise, nextPromise),
    ''
  );
}
